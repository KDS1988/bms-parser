...
function doPoll() {
  ...
  for (const { event, items } of entries) {
    ...
    if (isNewEvent) {
      ...
      try {
        appendNewEventRow_(event, items[0]); // project/customer/manager берём с первой попавшейся услуги
      } catch (e) {
    ...
    for (const item of targetItems) {
      ...
      if (!somethingChanged) continue; // ничего не поменялось — не спамим и не пишем лог повторно
      ...
      // Для только что созданного события уведомление уже отправлено выше ("🆕 Новое мероприятие") —
      // отдельное "🔄 Изменение" не дублируем. Иначе — если это новая услуга на уже известном
      // событии, или реально что-то поменялось в назначениях — шлём отдельное уведомление.
